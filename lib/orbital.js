...
Orbital.prototype.create = function create(obj,objname, cb){
	...
	request({uri:self.settings.url, body:doc.toString(),method:"POST", headers: headers}, function(err, response, body){
		...
		try{
			...
			console.log(body);
			var objresp = unmarshaller.unmarshalString(body).value;
			if(objresp.quickResp){
				return cb(new ChaseError(objresp.quickResp.procStatus, objresp.quickResp.statusMsg), null);
			}  
			return cb(null, camelCase(objresp));
		}catch(e){
			console.log(e);
			return cb(e, null);  
		}
...
Orbital.prototype.update = function updateProfile(obj,objname, cb){
	...
	obj.customerProfileAction = "U"; // set to update profile.	
	...
	request({uri:self.settings.url, body:doc,method:"POST", headers: headers}, function(err, response, body){
		...
		try{
			...
			return cb(null, camelCase(obj.profileResp));  
		}catch(e){
			console.log(e);
			return cb(e, null);  
		}
...
/**
 * Remove an object using the Orbital xml api
 * @param {object} obj The object to delete
 * @param {string} objname The Model name of the object to delete
 * @param {callback} cb The callback that receives the error and result
 */
Orbital.prototype.remove = function deleteProfile(obj, objname, cb){
	var self = this;
	...
	request({uri:self.settings.url, body:doc,method:"POST", headers: headers}, function(err, response, body){
		...
		cb(err, camelCase(obj.profileResp));
	})
}
