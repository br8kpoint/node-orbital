...
Orbital.prototype.create = function create(obj, cb){
	...
	if(typeof(obj.valid) == "function") obj.valid(function(err, cleaned){
		...
		var doc = new swiz.Swiz(obj._defs, {'stripNulls':false})
		doc.serialize(swiz.SERIALIZATION.SERIALIZATION_XML, 1, payload, function(err, serialized) {
		  ...
		  request({uri:self.settings.url, body:serialized,method:"POST", headers: headers}, function(err, response, body){
		  	...
		  	try{
				...
				sw.deserialize(swiz.SERIALIZATION.SERIALIZATION_XML, 1, body, function(err, objresp) {
					if(err){
						return cb(err, null);
					}
					if(obj.response && typeof(obj.response == "function")) return obj.response(objresp, self, cb);
					return cb(null, objresp);
				});	
		  	}catch(e){
		  		var sw = new swiz.Swiz(QuickResponse._defs, {stripNulls: true});
				sw.deserialize(swiz.SERIALIZATION.SERIALIZATION_XML, 1, body, function(err, objresp) {
					if(err){
						return cb(err, null);
					}
					if(obj.response && typeof(obj.response == "function")) return obj.response(objresp, self, cb);
					return cb(objresp, null);	// quick response is returned as error
				});	
		  	}
...
Orbital.prototype.update = function updateProfile(profile, cb){
	...
	var doc = new swiz.Swiz(profile._defs, {'stripNulls':false})
	doc.serialize(swiz.SERIALIZATION.SERIALIZATION_XML, 1, payload, function(err, serialized) {
		var headers = {
		  	'Content-Type':"application/PTI59",
		  	'MIME-Version':'1.1',
